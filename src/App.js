...
export default function QuantDashboard() {
  ...
  return (
    <div style={S.app}>
      <div style={S.hdr}>
        <div>
          <div style={{fontSize:9,color:"#aa88ff",letterSpacing:3}}>OPTIONS TRADING SYSTEM</div>
          <div style={{fontSize:18,fontWeight:"bold",color:"#fff"}}>Aidan's War Room</div>
          <div style={{fontSize:10,color:"#778899",marginTop:2}}>
            <span style={{color:"#aa88ff"}}>Robinhood Options</span>
            {" · "}Goal: <span style={{color:"#ffcc00"}}>$250 → $1K → $5K → $10K</span>
          </div>
        </div>
        <div style={{textAlign:"right"}}>
          <div style={{fontSize:9,color:"#444",letterSpacing:2}}>ACCOUNT VALUE</div>
          <div style={{fontSize:20,fontWeight:"bold",color:"#aa88ff"}}>{"$"}{totalValue.toFixed(2)}</div>
          <div style={{fontSize:11,color:"#778899"}}>
            Cash: <span style={{color:"#00ff88",fontWeight:"bold"}}>{"$"}{cashBalance.toFixed(2)}</span>
            {" · "}
            P&L: <span style={{color:totalPnL>=0?"#00ff88":"#ff4444",fontWeight:"bold"}}>{totalPnL>=0?"+":""}{totalPnL.toFixed(2)}</span>
          </div>
        </div>
      </div>
      ...
      <div style={S.tkr}>
        <div style={{display:"inline-flex",gap:16,alignItems:"center"}}>
          ...
          <button onClick={fetchLivePrices} style={{background:"none",border:"none",color:"#aa88ff25",cursor:"pointer",fontSize:13}}>⟳</button>
          {lastUpdated&&<span style={{color:"#445566",fontSize:9}}>{lastUpdated}</span>}
        </div>
      </div>
      ...
      <div style={{padding:14}}>
        ...
        {activeTab==="briefing"&&(
          <div>
            ...
            {briefing?(
              ...
              <div style={{textAlign:"center",padding:50,color:"#334455",border:"1px dashed #2a2a4a",borderRadius:4}}>
                <div style={{fontSize:28,marginBottom:8}}>🌍</div>
                <div style={{fontSize:10,letterSpacing:2,color:"#aa88ff"}}>GET MACRO CONTEXT BEFORE TRADING</div>
                <div style={{fontSize:10,color:"#445566",marginTop:6}}>Fed · Inflation · Geopolitics · Options Environment</div>
                <div style={{fontSize:10,color:"#334455",marginTop:4}}>Trade ideas → SCANNER tab</div>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab==="scanner"&&(
          <div>
            <div style={S.lbl}>AI OPTIONS SCANNER — 4-AGENT COMMITTEE</div>
            ...
            {scanResults.length>0&&(
              <div style={{marginTop:12}}>
                <div style={{color:"#aa88ff",fontSize:10,letterSpacing:2,marginBottom:12,padding:"6px 10px",background:"#0d0820",borderRadius:4,border:"1px solid #aa88ff30"}}>
            ...
            <div style={Object.assign({},S.card,{marginTop:20,border:"1px solid #1a1a3a"})}>
              <div style={S.lbl}>MANUAL SYMBOL CHECK</div>
              <div style={{fontSize:10,color:"#445566",marginBottom:8}}>Check a specific stock for an options play. All 4 agents analyze it.</div>
        ...
        {activeTab==="portfolio"&&(
          <div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:12}}>
              {[
                ["CASH","$"+cashBalance.toFixed(2),"#00ff88"],
                ["AT RISK","$"+totalPremiumAtRisk.toFixed(2),"#ff8844"],
                ["TOTAL","$"+totalValue.toFixed(2),"#aa88ff"]
              ].map(function(item){
            ...
            <div style={{marginBottom:12}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <span style={S.lbl}>OPEN OPTIONS POSITIONS ({optionsPositions.length})</span>
              </div>
              {optionsPositions.length===0?(
                React.createElement("div",{style:{textAlign:"center",padding:30,color:"#334455",border:"1px dashed #2a2a4a",borderRadius:4}},
                  ...
                  return React.createElement("div",{key:idx,style:{background:"#0a0e18",border:"2px solid "+uc+"40",borderRadius:8,padding:14,marginBottom:10,borderLeft:"4px solid "+uc}},
                    ...
                    React.createElement("div",{style:{display:"flex",gap:6}},
                      React.createElement("input",{
                        placeholder:"Close price (what you sold for e.g. 3.50)",
                        id:"close_"+idx,
                        style:Object.assign({},S.inp,{flex:1,fontSize:11})
                      }),
                      React.createElement("button",{
                        onClick:function(){
                          var cp=document.getElementById("close_"+idx).value;
                          closeOptionPosition(idx, cp||"0");
                        },
            ...
            <div style={Object.assign({},S.card,{border:"2px solid #aa88ff30"})}>
              <div style={S.lbl}>LOG OPTIONS TRADE (BUY TO OPEN)</div>
              <div style={{fontSize:10,color:"#445566",marginBottom:10}}>After executing on Robinhood, log it here to track it.</div>
              ...
              <div style={{display:"flex",gap:6,marginBottom:6}}>
                <input placeholder="Strike e.g. 220" type="number" value={optionForm.strike} onChange={function(e){ setOptionForm(Object.assign({},optionForm,{strike:e.target.value})); }} style={Object.assign({},S.inp,{flex:1})}/>
                <input placeholder="Expiry e.g. 2026-05-20" value={optionForm.expiry} onChange={function(e){ setOptionForm(Object.assign({},optionForm,{expiry:e.target.value})); }} style={Object.assign({},S.inp,{flex:1})}/>
              </div>
              <div style={{display:"flex",gap:6,marginBottom:6}}>
                ...
                <input placeholder="Premium paid e.g. 1.50" type="number" value={optionForm.premium} onChange={function(e){ setOptionForm(Object.assign({},optionForm,{premium:e.target.value})); }} style={Object.assign({},S.inp,{flex:1})}/>
              </div>
              {optionForm.contracts&&optionForm.premium&&React.createElement("div",{style:{background:"#0d0820",borderRadius:4,padding:"8px 10px",marginBottom:8,border:"1px solid #aa88ff20",display:"flex",justifyContent:"space-between",alignItems:"center"}},
                ...
                React.createElement("span",{style:{color:parseFloat(optionForm.contracts||1)*parseFloat(optionForm.premium||0)*100>cashBalance?"#ff4444":"#ffcc00",fontWeight:"bold",fontSize:15}},"$"+(parseInt(optionForm.contracts||1)*parseFloat(optionForm.premium||0)*100).toFixed(0)),
                React.createElement("span",{style:{color:"#445566",fontSize:10}},"of $"+cashBalance.toFixed(2)+" available")
              )}
              <input placeholder="Note e.g. Scanner approved 3/4 bullish MACD crossover" value={optionForm.note} onChange={function(e){ setOptionForm(Object.assign({},optionForm,{note:e.target.value})); }} style={Object.assign({},S.inp,{marginBottom:8})}/>
              <button onClick={logOptionTrade} style={S.btn()}>CONFIRM — LOG OPTIONS TRADE</button>
            </div>

            <div style={Object.assign({},S.card,{marginTop:8,border:"1px solid #1a2a3a"})}>
              <div style={S.lbl}>SET CASH BALANCE</div>
              <div style={{fontSize:10,color:"#445566",marginBottom:8}}>Update when you add or withdraw funds from Robinhood.</div>
              <div style={{display:"flex",gap:8}}>
                <input placeholder="Enter cash balance e.g. 250" type="number" id="cashInput" style={Object.assign({},S.inp,{flex:1})}/>
        ...
        {activeTab==="risk"&&(
          <div>
            <div style={S.lbl}>RISK MANAGEMENT</div>
            ...
            <div style={S.card}>
              <div style={S.lbl}>OPTIONS RULES</div>
              {[
                {label:"Max per trade",rule:"$"+Math.min(cashBalance*0.4,300).toFixed(0)+" (40% of cash)",status:"RULE"},
                {label:"Contracts per trade",rule:"1 contract until $1,000",status:"RULE"},
                {label:"Min agent votes",rule:"2/4 agents must agree",status:"RULE"},
                {label:"Max expiry",rule:"7 days out maximum",status:"RULE"},
                {label:"Take profit at",rule:"50%+ gain — don't get greedy",status:"RULE"},
                {label:"Never hold to expiry",rule:"Sell the day before at latest",status:"RULE"},
              ].map(function(r){
            ...
            <div style={S.card}>
              ...
              {riskAlerts.length===0?
                React.createElement("div",{style:{color:"#aa88ff66",fontSize:12,textAlign:"center",padding:20}},"All clear — no active risk alerts")
            ...
            <div style={S.card}>
              <div style={S.lbl}>MILESTONE TRACKER</div>
              {[[250,1000],[1000,5000],[5000,10000]].map(function(m){
                ...
                return React.createElement("div",{key:from,style:{marginBottom:10,padding:"8px 10px",background:active?"#0d0820":"#04060e",borderRadius:4,border:active?"1px solid #aa88ff30":"1px solid #08080a"}},
                  React.createElement("div",{style:{display:"flex",justifyContent:"space-between",marginBottom:4}},
                    React.createElement("span",{style:{color:done?"#00ff88":active?"#aa88ff":"#445566",fontSize:11,fontWeight:"bold"}},done?"✓ ":"","$"+from+" → $"+to),
        ...
        {activeTab==="trades"&&(
          <div>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <span style={S.lbl}>TRADE HISTORY ({trades.length})</span>
              <span style={{color:totalPnL>=0?"#00ff88":"#ff4444",fontSize:12,fontWeight:"bold"}}>Total P&L: {totalPnL>=0?"+":""}{totalPnL.toFixed(2)}</span>
            </div>
            {trades.length===0?
              React.createElement("div",{style:{textAlign:"center",padding:40,color:"#334455",border:"1px dashed #2a2a4a",borderRadius:4}},
                React.createElement("div",{style:{fontSize:24,marginBottom:8}},"📋"),
                ...
                return React.createElement("div",{key:idx,style:{background:"#0a0e18",border:"1px solid #1a1a2a",borderRadius:6,padding:12,marginBottom:8,borderLeft:"3px solid "+bc}},
                  React.createElement("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}},
                    ...
                    React.createElement("div",{style:{display:"flex",gap:6,alignItems:"center"}},
                      ...
                      React.createElement("button",{onClick:function(){ startEditTrade(idx); },style:{background:"#0a1428",border:"1px solid #4488ff60",color:"#4488ff",padding:"2px 7px",fontSize:9,cursor:"pointer",borderRadius:3,fontFamily:"inherit"}},"EDIT"),
                      React.createElement("button",{onClick:function(){ if(window.confirm("Delete this trade?")) deleteTrade(idx); },style:{background:"#1a0606",border:"1px solid #ff444460",color:"#ff4444",padding:"2px 7px",fontSize:9,cursor:"pointer",borderRadius:3,fontFamily:"inherit"}},"DELETE")
                    )
                  ),
                  isEditing?(
        ...
        {activeTab==="chat"&&(
          <div>
            ...
            <div style={{fontSize:10,color:"#445566",marginBottom:10}}>Ask anything — options plays, stock analysis, market questions, strategy. Your account size and positions are always known.</div>
            <div style={{background:"#04060e",border:"1px solid #0a0a1a",borderRadius:4,padding:12,minHeight:220,maxHeight:420,overflowY:"auto",marginBottom:10}}>
              {chatHistory.length===0?
                React.createElement("div",{style:{color:"#334455",fontSize:10,letterSpacing:2,textAlign:"center",marginTop:70}},
                  React.createElement("div",{style:{fontSize:24,marginBottom:8}},"💬"),
